...
int Solver::data_reduction(int k, int c, int layer){
    int k_before = k;
...
int Solver::data_reduction_before_branching(){
    ...
    while(k_tmp != k_before){
        k_before = k_tmp;
        k_tmp = dataRed_heavy_non_edge_branch(k_tmp, 0);
        k_tmp = dataRed_heavy_edge_single_end_branch(k_tmp, 0);
        k_tmp = dataRed_heavy_edge_both_ends(k_tmp, 0);
...
int Solver::dataRed_weight_larger_k(int k, int c){
    ...
    for(int u : g->active_nodes[c]){
        for(int v : g->active_nodes[c]){
            if(u == v) continue;
...
int Solver::dataRed_heavy_non_edge_branch(int k, int c) {
    redo:
    if(k < 0) {
        return -1;
    }
...
int Solver::dataRed_heavy_edge_single_end_branch(int k, int c) {
    redo:
    if(k < 0) {
//        printDebug("Fail: maximum cost exceeded");
        return -1;
...
int Solver::dataRed_heavy_edge_both_ends(int k, int c) {
    int cost = 0;
    redo:
    if(k < 0) {
//        printDebug("Fail: maximum cost exceeded");
...
//is doing the large Neighbourhood Rule for all vertices in the graph
int Solver::dataRed_large_neighbourhood_I(int k, int c) {
        ...
        if(deficiency != DO_NOT_DELETE && cut_weight != DO_NOT_DELETE) {
            ...
            if(sum < neighbours.size() && neighbours.size() >= 2){
                ...
                while(!neighbours.empty()){
                    int last_merged = g->active_nodes[c].back();
...
void Solver::dataRed_split_component(int c){
    std::vector<std::vector<int>> components = find_components(c);

    if (components.size() > 1) {
        printDebug("Split componrnt sucess");
        g->split_component(components);
    }
}

std::vector<std::vector<int>> Solver::find_components(int c) {
    int N = g->merge_map.size();
    bool *visited = new bool[N];
